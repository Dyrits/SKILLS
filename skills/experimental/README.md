...
- **[route](./route/SKILL.md)**: Classify an incoming request by task type and complexity, then delegate it to the subagent that fits, so a classifier picks the agent and its pinned model rather than the agent's own judgement.
