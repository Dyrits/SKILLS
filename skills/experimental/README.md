...
- **[dispatch](./dispatch/SKILL.md)**: Bind a route recommendation (task type, complexity), normally supplied automatically by `setup-routing-for-claude`'s hook, to the subagent that fits, brief it, and dispatch, so a classifier picks the agent and its pinned model rather than the agent's own judgement.
