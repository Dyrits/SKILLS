...
- **[setup-auto-route](./setup-auto-route/SKILL.md)**: Install a Claude Code UserPromptSubmit hook that classifies every prompt with classifier.dev and speaks only when the verdict changes, writing a route recommendation for the `dispatch` skill. (User-invoked, not in the plugin.)
