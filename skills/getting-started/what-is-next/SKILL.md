...
- **Subagent**: send a tightly-scoped task to its own window and get a report back. **`/route`** (under Standalone) picks which subagent by classifier rather than by feel.
...
- **`/route`** classifies a fresh unit of work before anything is done to it (task type, then complexity when it's a code change) and dispatches it to the subagent that fits, so the agent and its pinned model come from a classifier rather than from the agent's own judgement. It routes at the **top** only: on a turn that builds on what's already in this window, continuing here beats handing a subagent a request without its context. Experimental, because the request text goes to a third-party API.
- **`/classify`** is the primitive underneath `/route`: your text, your labels, one label back with a **calibrated** confidence. Reach for it wherever you'd otherwise read N items to keep a few (triage, filter, bucket a search result set) or to settle a choice between named options: `/classify <text> [option, option, option]`. Experimental for the same reason.
