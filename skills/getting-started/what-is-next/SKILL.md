...
- **`/publish-message`** publishes a conclusion already reached in the conversation as a short tracker comment; **`/publish-review`** publishes a finished `/code-review` report as a summary and inline suggestions. Both are experimental because they post to tracker threads.
