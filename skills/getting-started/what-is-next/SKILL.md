...
- **Subagent**: send a tightly-scoped task to its own window and get a report back. **`/dispatch`** (under Standalone) picks which subagent by classifier rather than by feel. Install **`/setup-auto-route`** once and a recommendation lands in context automatically whenever the classified route changes, so `/dispatch` has one waiting rather than needing a classifier call first.
