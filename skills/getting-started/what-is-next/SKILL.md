...
1. **`/grill-with-documentation`** sharpens the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grill-me` instead, covered under Standalone. Both run the same `/grilling` primitive; `grill-with-documentation` is the one that leaves a paper trail, which makes it the better of the two whenever a repository is there to leave it in.)
...
- **`/domain-modeling`**: sharpen the project's *domain* language: challenge a fuzzy term, resolve an overloaded word ("account" doing three jobs), record a hard-to-reverse decision as an ADR. It's the active discipline `/grill-with-documentation` drives to keep `CONTEXT.md` a clean glossary.
...
- **Subagent**: send a tightly-scoped task to its own window and get a report back. **`/dispatch`** (under Standalone) picks which subagent by classifier rather than by feel. Install **`/setup-routing-for-claude`** once and a recommendation lands in context automatically whenever the classified route changes, so `/dispatch` has one waiting rather than needing a classifier call first.
...
- **`/grill-me`**: the same relentless interview as `/grill-with-documentation`, but **stateless**: it saves nothing locally and builds no `CONTEXT.md`. Reach for it when you are **not working in a working directory** (sharpening a plan, a design, a piece of writing, anything with no repository under it). If you are in a working directory, use `/grill-with-documentation` instead: it runs the same interview and leaves a paper trail, so it is strictly the better one.
...
- **`/dispatch`** binds a route recommendation (task type, then complexity when it's a code change) to the subagent that fits, briefs it, and dispatches, so the agent and its pinned model come from a classifier rather than from the agent's own judgement. It routes at the **top** only: on a turn that builds on what's already in this window, continuing here beats handing a subagent a request without its context. Experimental, because the request text goes to a third-party API. Its recommendation normally arrives on its own: see `/setup-routing-for-claude` below.
- **`/classify`** is the primitive `/dispatch`'s hook re-implements in bash: your text, your labels, one label back with a **calibrated** confidence. Reach for it wherever you'd otherwise read N items to keep a few (triage, filter, bucket a search result set) or to settle a choice between named options: `/classify <text> [option, option, option]`. Experimental for the same reason.
- **`/setup-routing-for-claude`** installs a `UserPromptSubmit` hook that classifies every prompt automatically, before any reasoning starts, and speaks only when the verdict changes, so `/dispatch` finds a recommendation already waiting in context instead of calling `/classify` itself. Run once per repository; a restart is needed before it takes effect.
