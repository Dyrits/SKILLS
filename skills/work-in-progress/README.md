...
- **[loop-me](./loop-me/SKILL.md)**: Grill yourself into implementable workflow specifications over multiple sessions, using the current directory as a stateful workspace. User-invoked.
...
- **[setup-ts-deep-modules](./setup-ts-deep-modules/SKILL.md)**: Wire dependency-cruiser into a TypeScript repository so each package is a deep module: implementation hidden in subfolders, reachable only through its entry-point files, tests exercising it through those. User-invoked.
