...
- **[dispatch](./skills/experimental/dispatch/SKILL.md)**: Bind a route recommendation (task type, complexity) to the subagent that fits, brief it, and dispatch, so a classifier picks the agent and its pinned model rather than the agent's own judgement.
