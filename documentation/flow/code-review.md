...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-custom-skills](https://aihero.dev/skills-setup-custom-skills) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
[what-is-next](https://aihero.dev/skills-what-is-next) routes across the whole set when you are unsure which skill the situation wants.
