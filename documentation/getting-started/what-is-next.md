...
- **The main flow**, idea to ship. Grill, specification, tickets, implement, review, with two branches inside it: a prototype detour when a question needs runnable code to settle, and the specification-and-tickets split, which only earns its cost when the build spans more than one session.
