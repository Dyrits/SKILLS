...
`ask-someone-else` turns a decision you can't settle on your own into a **questionnaire**: a Markdown document you hand to the one person who holds what you're missing, for them to fill in async or for the two of you to work through in a meeting.
...
You invoke this by typing `/ask-someone-else`; the [agent](https://www.aihero.dev/ai-coding-dictionary/agent) won't reach for it on its own.
...
The common case is a [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling) session that stalls: some of what surfaced isn't yours to answer. Run `/ask-someone-else` in that same conversation to take those questions offline, then bring the answers back and carry on.
...
Everything after that is drafting. The file lands at `ask-someone-else-<slug>.md` in the current directory. There is no setup, no workspace, and nothing to configure.
