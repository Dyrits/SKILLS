...
The Specification axis needs a specification to exist and be findable. It looks in this order:
...
| Reads | The repository's documented standards, plus the smell baseline | The originating issue or specification |
...
| Every finding cites | The standards file and the rule, or the named smell plus the hunk | The line of the specification |

A generic review skill that does not know your standards is the thing this design is trying to avoid: it flags what is deliberate in your codebase and misses the invariants your codebase actually depends on. So the repository's own documentation is the primary source on the Standards axis, and **the repository always overrides**.
...
- Every Standards finding names either a rule in one of your repository's files or one of the twelve smells, with the hunk quoted; every Specification finding quotes a line of the specification.
- The closing summary gives a worst issue per axis and declines to pick an overall winner.
- With no specification available, the Specification block says so instead of listing requirements it inferred from the code.
