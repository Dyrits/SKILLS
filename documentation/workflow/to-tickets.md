...
Before drafting, `to-tickets` reads the configured ticket-writing convention and asks whether to apply it. The convention may be the built-in format, a repository issue-template document, or a ticket-writing skill such as `create-jira-ticket`. Selecting a skill borrows its ticket structure, language, and quality rules; it does not hand over decomposition or publication.

The tracer-bullet rules always remain in force. Every resulting ticket still describes a complete end-to-end behavior, declares its blockers, and carries acceptance criteria, even when a team template gives those fields different headings.
...
**On GitHub the tickets weren't created as sub-issues of the specification issue.**
...
- The agent names the ticket-writing convention it will apply, and you can choose the built-in format instead.
