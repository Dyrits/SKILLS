...
You cannot preserve an intent you have not read. So the work starts in the history (commits, PRs, tickets) and only then moves to the diff. Another step in the loop exists for the same reason: the skill finds the repository's own automated checks and runs them before committing, because a merge is the easiest place in git to produce code that satisfies both branches and passes neither's tests.
