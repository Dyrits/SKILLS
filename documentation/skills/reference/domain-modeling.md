...
Ask for it explicitly rather than waiting for it to accumulate. `/grill-with-documentation help me scaffold my existing repository with a CONTEXT.md` is the documented route; expect a long interrogation: one user reported 50+ questions before the file was in shape. Incidental use builds the glossary far too slowly on a brownfield repository.
