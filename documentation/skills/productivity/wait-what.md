...
`wait-what` is what you type when a message didn't land. The agent then re-pitches what it just said. It adds the context you were missing, writes in plain English, and uses the vocabulary from your project's `CONTEXT.md`. If you have been writing in another language, the re-pitch comes back in that language; if your preference isn't clear, it asks before answering.
...
Use it the second you notice you're skimming. The agent has drifted into jargon it invented, stacked five acronyms, or explained a decision whose premise you never saw. It fixes the conversation you're already in. To stop the jargon arriving at all, use [grill-with-documentation](../workflow/grill-with-documentation.md), which builds the shared language upfront.
...
The body reuses the leading words already in your global `CLAUDE.md` and your project's `CONTEXT.md`. ASD-STE100 Simplified Technical English sets the register when the answer is in English (STE is an English-only register; other languages get the same plain-technical intent in their own idiom). The ubiquitous language supplies the nouns. The skill, `CLAUDE.md` and `CONTEXT.md` reach for the same tokens, so invoking it is not a new instruction. It is a reminder of one the agent already agreed to.
...
You can use `wait-what` at any point, in any conversation, inside any other skill. It repairs one message after the fact. The real cure is a shared language agreed upfront, and that is [grill-with-documentation](../workflow/grill-with-documentation.md): a grilling session that runs [domain-modeling](../reference/domain-modeling.md) as it goes, so the words you both use land in your `CONTEXT.md`. If you're unsure which skill fits the moment, [what-is-next](../getting-started/what-is-next.md) routes you.
