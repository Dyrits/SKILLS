...
The map and its tickets live on the repository's issue tracker, so wayfinder needs the tracker wiring that [setup-custom-skills](../getting-started/setup-custom-skills.md) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
| --- | --- | --- | --- |
| `grilling` | HITL | The default. The question can be settled by talking it through. | [grilling](../reference/grilling.md) plus [domain-modeling](../reference/domain-modeling.md), in a fresh session |
...
**The map is cleared. Didn't wayfinder already write the specification and make the tickets? Why do I still need `/to-specifications` and `/to-tickets`?**
No. Wayfinder's tickets are decision tickets, and by the time the map closes they are all closed too. What is left is a map full of linked decisions, which is not a build plan. [to-specifications](../workflow/to-specifications.md) collapses those linked decisions into one specification (`/to-specifications #<map_issue>`) and [to-tickets](../workflow/to-tickets.md) slices that into tracer-bullet implementation tickets. Looping the map straight into [implement](../workflow/implement.md) skips the collapse and throws the linked detail away. Go straight to implementation only when the effort turned out genuinely small. People do run the abbreviated pipeline and report it working; the two extra steps buy you an explicit specification artifact that a reviewer or a colleague can read, which matters more the less solo you are.
...
- The session that finishes the map hands you toward a specification, not a pull request.
