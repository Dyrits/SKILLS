...
| You want the whole plan interrogated before you build | [grill-with-docs](../workflow/grill-with-docs.md), which drives this skill underneath |
...
- **`CONTEXT.md`** at the repository root, created by the first resolved term. In a repository with a `CONTEXT-MAP.md` at the root, terms go into the per-context `CONTEXT.md` the map points at instead.
...
| | `CONTEXT.md` | `documentation/architecture-decision-record/NNNN-slug.md` |
| --- | --- | --- |
...
The `CONTEXT.md` rule is the one to actually hold onto, because it is the one that breaks in the field. **It is a glossary and nothing else.** Left unchecked, models treat "write to `CONTEXT.md`" as permission to persist every answer you give, and the file turns into a running specification. This is the most-reported problem with the skill, across several models.
...
Ask for it explicitly rather than waiting for it to accumulate. `/grill-with-docs help me scaffold my existing repository with a CONTEXT.md` is the documented route; expect a long interrogation: one user reported 50+ questions before the file was in shape. Incidental use builds the glossary far too slowly on a brownfield repository.
